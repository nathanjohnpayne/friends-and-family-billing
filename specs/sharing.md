...
- Pending charges section (deferred Usage Charges, #317) renders a list of the member's own deferred charges (date, description, amount, running total) with a clear NOT-YET-DUE label, only when the resolved share data carries a non-empty `pendingCharges` (which the Cloud Function returns only for the `usageCharges:read` scope); hidden when the scope is absent or there are no deferred charges. The explanatory note states both dispositions of a pending charge — it will either be billed separately by the admin (the off-cycle Charge Notice path, #320) or applied to the member's bill next year (the carry-forward seam, #322) (#338).
