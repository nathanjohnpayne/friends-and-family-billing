...
- `buildShareScopes` always includes `summary:read`, `paymentMethods:read`, `usageCharges:read` (#317 — a member always sees their own pending charges on their share, so the scope is on every normal link), and `payments:read` (#356 — a member's payment history is their own data, the same posture as `usageCharges:read`); adds `disputes:create` and/or `disputes:read` when the respective flags are true; and adds `refunds:read` when the third flag is true (#319). The 2-argument dispute call stays backwards-compatible. Links minted before `payments:read` existed simply won't carry it, so the share page degrades gracefully (omits payment history).
- `buildPublicShareData` includes the member's `pendingCharges` (via `buildPendingChargesForShare`) in the generated `publicShares` document when the scopes carry `usageCharges:read`, so the member-facing view is reachable directly from a normally-generated link (not only via the Cloud Function self-heal on a cache miss). It accepts an optional `owedAdjustments` array for this. The React path threads `owedAdjustments` through every share-generation caller (`ShareLinkService`, the ShareLink/Email/Text dialogs, and `InvoicingTab`) for cache-hit consumer parity with the Cloud Function fallback.
- The same `owedAdjustments` array drives the member-facing **owed** (#321 + #322): `buildPublicShareData` composes the household's `combinedAnnualTotal` (and therefore `combinedMonthlyTotal` and `balanceRemaining`) as `Math.max(0, combinedAnnual − serviceCreditTotal + openingBalance)`, where `serviceCreditTotal` is the sum of active `service_credit` adjustments for the primary plus linked members and `openingBalance` is the household's netted carried opening balance — the signed sum of `carry_opening` seed records (a carried **credit** is negative and lowers owed, a carried **charge** positive and raises it) for the primary plus linked members, via `getHouseholdOpeningBalance`. The combined result is floored at 0, mirroring `getHouseholdFinancials` and the invoice so the public share summary agrees with the settlement board. Voided credits/seeds and the `+owed` Usage Charge direction are excluded; billed Usage Charges (#320) are not added here. `carry_opening` is a distinct `kind`, so the service-credit sum ignores it (no double-count). The Cloud Function `resolveShareToken` — the cache-miss / stale-refresh fallback that self-heals `publicShares` — applies the same floored `service_credit` reduction **and** carried-opening-balance adjustment to its `paymentSummary` (via the `getServiceCreditTotalForMember` and `getHouseholdOpeningBalance` helpers in `functions/billing.js`), so the React and Cloud Function member-facing totals agree and the self-healed doc never persists an uncarried (or gross) total.
...
- `buildPaymentHistoryForShare` (#356) returns the household's (the token member plus their linked members — the same grain as `totalPaid`) settled payments as member-safe line items: `{ payments: [{ id, date, amount, method }], count }`, newest first. Only "live" ledger entries are projected — reversal entries (`type: 'reversal'`) and reversed originals (`reversed === true`) are excluded, so the items sum to the same combined `totalPaid` that `getPaymentTotalForMember` produces (an original and its reversal net to zero). Only member-safe fields are exposed (`id`, `date` = the payment's `receivedAt`, `amount`, `method`); the free-text `note` is never included. `buildPublicShareData` includes `paymentHistory` in the generated `publicShares` document when the scopes carry `payments:read`, and the Cloud Function `resolveShareToken` mirrors the same field (via the `functions/billing.js` mirror) into both its response and the self-healed doc, so the cache-hit, cache-miss, and self-healed doc all agree (an explicit cache↔CF parity test asserts the two builders produce identical output). The history adds line items alongside the existing `totalPaid` / `balanceRemaining`; it does not recompute them.
- `buildServiceCreditsForShare` returns the household's active Service Credits (#337) as member-safe line items for the share summary: `{ items: [{ reason, billName, amount }], total }`. Active `service_credit` records for the primary plus linked members are aggregated by (billId + reason) so a bill-level split (one record per affected member) shows as a single line; the `+owed` Usage Charge direction, voided credits, `carry_opening` seeds, and other members' credits are excluded; only member-safe fields are exposed. The `total` equals the `service_credit` reduction already folded into `combinedAnnualTotal`. `buildPublicShareData` includes `serviceCredits` in the generated `publicShares` document under `summary:read` when the total is positive, and the Cloud Function `resolveShareToken` mirrors the same field (via the `functions/billing.js` mirror) so the cache-hit, cache-miss, and self-healed doc all agree.
...
- Payment history section (#357) renders the household's settled payments from the `payments:read` payload (`paymentHistory`, built by `buildPaymentHistoryForShare`) behind a collapsed "Show payment history ({count})" entry point that expands to a date / method / amount list (method via `getPaymentMethodLabel`, amount via `formatCurrency`). Only member-safe fields are shown — never the free-text note. The section degrades gracefully: it is omitted entirely when the resolved data carries no `paymentHistory` (older links that predate `payments:read`) or an empty list.
- Disputes section renders dispute details (bill name, status, message, resolution note); hidden when empty.
- Pending charges (deferred Usage Charges, #317) render as a compact amber **notice** (mockup redesign) placed right after the bill/summary (not at the bottom of the page): a "{count} upcoming charge(s) — not yet due" header, one line per charge (amount · description · incurred date), and a total — only when the resolved share data carries a non-empty `pendingCharges` (which the Cloud Function returns only for the `usageCharges:read` scope); hidden when the scope is absent or there are no deferred charges. The notice states both dispositions of a pending charge — it will either be billed separately by the admin (the off-cycle Charge Notice path, #320) or applied to the member's bill next year (the carry-forward seam, #322) (#338).
- Service credits reconciliation (#337): when the resolved data carries `serviceCredits`, the Household's Annual Billing Summary renders a reconciliation under the household total — the bills subtotal, each Service Credit as a `-$amount` line (`Service credit — {billName} ({reason})`), an "Other adjustments" line for any residual (a carried opening balance, #322, or the owed floored at 0 when credits exceed the bills), and the resulting Household Total — so the reduced annual total is explained rather than silently lower than the sum of the bills. Hidden when there are no active service credits.
