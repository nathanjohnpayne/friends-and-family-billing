...
- `totalCreditsOwed` is the sum of unresolved household credits and is tracked on a separate axis from `totalOutstanding`; an overpaid household still counts as settled (its surplus does not block settlement), and a household assigned to no bill has zero owed and likewise counts as settled, so it never blocks the Ready-to-Close state.
