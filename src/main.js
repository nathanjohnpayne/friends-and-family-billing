...
import {
    getBillAnnualAmount as _getBillAnnualAmount,
    getBillMonthlyAmount as _getBillMonthlyAmount,
    calculateAnnualSummary as _calculateAnnualSummary,
    getPaymentTotalForMember as _getPaymentTotalForMember,
    getMemberPayments as _getMemberPayments,
...
import {
    ...
    generateUniqueBillId as _generateUniqueBillId,
    isArchivedYear as _isArchivedYear,
    isClosedYear as _isClosedYear,
    isSettlingYear as _isSettlingYear,
    isYearReadOnly as _isYearReadOnly,
    yearReadOnlyMessage as _yearReadOnlyMessage,
...
// ──────────────── Wrappers for parameterized lib functions ────────────────
// These pass module-scoped state to the pure lib versions so existing
// callers in this file don't need to change.

function isArchivedYear() { return _isArchivedYear(currentBillingYear); }
function isClosedYear() { return _isClosedYear(currentBillingYear); }
function isSettlingYear() { return _isSettlingYear(currentBillingYear); }
function isYearReadOnly() { return _isYearReadOnly(currentBillingYear); }
function yearReadOnlyMessage() { return _yearReadOnlyMessage(currentBillingYear); }
function generateUniqueId() { return _generateUniqueId(familyMembers.map(m => m.id)); }
function generateUniqueBillId() { return _generateUniqueBillId(bills.map(b => b.id)); }
function getBillAnnualAmount(bill) { return _getBillAnnualAmount(bill); }
...
// isArchivedYear, isClosedYear, isSettlingYear, isYearReadOnly,
// yearReadOnlyMessage, BILLING_YEAR_STATUSES, getBillingYearStatusLabel
// — moved to lib/validation.js and lib/formatting.js; wrappers above
...
async function closeCurrentYear() {
    ...
    // owedAdjustments threads BILLED usage charges (#320, raise Outstanding → block
    // close), Service Credits (#321, lower owed), and carried opening balances (#322)
...
// escapeHtml, sanitizeImageSrc, getInitials — moved to lib/formatting.js
// isValidE164 — moved to lib/validation.js
...
// generateUniqueId, generateUniqueBillId — moved to lib/validation.js; wrappers above
...
// isLinkedToAnyone, getParentMember — moved to lib/calculations.js; wrappers above
...
// generateEventId — moved to lib/validation.js
...
// BILLING_EVENT_LABELS — moved to lib/formatting.js

// ──────────────── Billing Frequency Helpers ─────────────────────
// getBillAnnualAmount, getBillMonthlyAmount, getBillFrequencyLabel
// — moved to lib/calculations.js and lib/formatting.js; wrappers above
...
// calculateAnnualSummary — moved to lib/calculations.js; wrapper above
...
// calculateSettlementMetrics — moved to lib/calculations.js; wrapper above
...
// PAYMENT_PROVIDER_PATTERN, detectDuplicatePaymentText — moved to lib/validation.js
...
// PAYMENT_METHOD_ICONS, PAYMENT_METHOD_STRIP_ICONS, getPaymentMethodIcon,
// getPaymentMethodStripIcon, PAYMENT_METHOD_TYPES — moved to lib/formatting.js
...
// getPaymentMethodDetail — moved to lib/formatting.js
...
// getEnabledPaymentMethods — wrapper above
// formatAnnualSummaryCurrency — moved to lib/formatting.js
...
// DISPUTE_STATUS_LABELS — moved to lib/formatting.js
// normalizeDisputeStatus — moved to lib/validation.js
// disputeStatusClass — moved to lib/formatting.js
...
// formatFileSize — moved to lib/formatting.js
...
// hashToken, generateRawToken — moved to lib/validation.js
...
function getInvoiceSummaryContext(memberId) {
    ...
    // Compose both member-facing owed modifiers in ONE floored expression (parity with
    // the React invoice.js and getHouseholdFinancials, dual-app parity): active Service
    // Credits (#321) subtract and the netted carried opening balance (#322) adds (a
    // carried credit lowers the first invoice total, a carried charge raises it).
    // Floored at 0 so neither an over-large credit nor a carried credit shows a negative.
    const serviceCreditTotal = getServiceCreditTotalForMember(memberId)
...
// generateUniquePaymentId — moved to lib/validation.js
// getPaymentTotalForMember, getMemberPayments — moved to lib/calculations.js; wrappers above
