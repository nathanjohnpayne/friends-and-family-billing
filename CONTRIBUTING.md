...
1. Branch from `main`
2. Run `npm test` — all tests (1029 React + 58 Cloud Functions) must pass before opening a PR
...
This runs the React test suite (1029 tests) and the Cloud Functions test suite (58 tests in `tests/functions.test.js`) plus a tracked-file secret scan. Playwright E2E tests are available separately via `npm run test:e2e`.
