...
function generateIndividualInvoiceHTML(memberData, currentYear) {
    ...
    let html = `
        ...
        <html>
        <head>
            <title>Annual Bill Invoice - ${member.name} - ${currentYear}</title>
            <style>
                body {
                    font-family: Arial, sans-serif;
                    max-width: 800px;
        ...
        <body>
            <h1>${avatarHTML}Annual Bill Invoice for ${member.name}</h1>
            <p>Year: ${currentYear}</p>
            <p>Generated on: ${new Date().toLocaleDateString()}</p>

            <button class="no-print" onclick="window.print()" style="padding: 10px 20px; background: #667eea; color: white; border: none; border-radius: 5px; cursor: pointer; margin: 20px 0;">Print as PDF</button>
    ...
    html += `
                    <tr class="total-row">
                        <td colspan="3">TOTAL</td>
                        <td>$${(memberData.total / 12).toFixed(2)}</td>
                        <td><strong>$${memberData.total.toFixed(2)}</strong></td>
                    </tr>
                </tbody>
            </table>
